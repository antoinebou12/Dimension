//! WASM platform: canvas, requestAnimationFrame, geometry scene, orbit + zoom.
...
pub fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let engine_cell: Rc<RefCell<Option<Engine>>> = Rc::new(RefCell::new(None));
    ...
    let engine_clone = Rc::clone(&engine_cell);
    ...
    wasm_bindgen_futures::spawn_local(async move {
        match Engine::new_wasm(canvas.clone()).await {
            Ok(mut eng) => {
                let _ = build_geometry_scene(eng.world_mut());
                let _ = eng.enable_ui();
            ...
            Err(e) => {
                let msg = format!("geometry-demo init failed: {e}");
...
fn setup_listeners(
    engine_cell: &Rc<RefCell<Option<Engine>>>,
    orbit_state: &Rc<RefCell<OrbitState>>,
    canvas: &HtmlCanvasElement,
) {
    let engine_cell = Rc::clone(engine_cell);
    let canvas = canvas.clone();

    let engine_down = Rc::clone(&engine_cell);
    ...
    let on_pointer_down: Closure<dyn FnMut(web_sys::PointerEvent)> =
        Closure::new(move |e: web_sys::PointerEvent| {
            ...
            if let Some(eng) = engine_down.borrow_mut().as_mut() {
    ...
    let engine_up = Rc::clone(&engine_cell);
    ...
    let on_pointer_up: Closure<dyn FnMut(web_sys::PointerEvent)> =
        Closure::new(move |_e: web_sys::PointerEvent| {
            if let Some(eng) = engine_up.borrow_mut().as_mut() {
                eng.ui_mouse_up();
            }
    ...
    let engine_move = Rc::clone(&engine_cell);
    ...
    let engine_wheel = Rc::clone(&engine_cell);
    let on_wheel: Closure<dyn FnMut(web_sys::WheelEvent)> =
        Closure::new(move |e: web_sys::WheelEvent| {
            ...
            if let Some(eng) = engine_wheel.borrow_mut().as_mut() {
                eng.zoom(-delta);
            }
        });
    canvas
    ...
    let engine_resize = Rc::clone(&engine_cell);
    ...
    let on_resize: Closure<dyn FnMut()> = Closure::new(move || {
        ...
        if w > 0 && h > 0 {
            if let Some(eng) = engine_resize.borrow_mut().as_mut() {
                eng.resize(w, h);
            }
        }
    });
...
fn schedule_frame(
    ...
    let closure = Closure::once(move || {
        let now_ms = web_sys::window()
            ...
            .map(|p| p.now())
            .unwrap_or(0.0);
        let mut g = engine_cell.borrow_mut();
        if let Some(eng) = g.as_mut() {
        ...
        schedule_frame(&engine_cell, &frame_timing);
    });
    web_sys::window()
        .and_then(|w| {
