...
use mathlib::{Graph, dsatur_coloring, greedy_vertex_coloring, is_bipartite};
...
#[test]
fn dsatur_often_fewer_colors_than_greedy() {
    ...
    let greedy_colors = greedy_vertex_coloring(&g);
    let dsatur_colors = dsatur_coloring(&g);
    let greedy_max = greedy_colors.iter().copied().max().unwrap_or(0);
    let dsatur_max = dsatur_colors.iter().copied().max().unwrap_or(0);
    assert!(dsatur_max <= greedy_max + 1); // DSatur should not use more colors than greedy (often fewer)
}
