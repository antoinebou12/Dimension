...
/** Default padding (px) for canvas drawing; use for margins in all chart/grid helpers. */
...
export function getCanvasThemeColors() {
  try {
    ...
    return {
      bg: (s.getPropertyValue("--canvas-bg") || "#fff").trim(),
      stroke: (s.getPropertyValue("--canvas-stroke") || "#212529").trim(),
      barPos: (s.getPropertyValue("--canvas-bar-pos") || "#0d6efd").trim(),
      barNeg: (s.getPropertyValue("--canvas-bar-neg") || "#fd7e14").trim(),
    };
  } catch (_) {
    return { bg: "#fff", stroke: "#212529", barPos: "#0d6efd", barNeg: "#fd7e14" };
  }
}
...
export function graphNodePositions(n, width, height, padding) {
  const cx = width / 2;
  const cy = height / 2;
  const r = Math.min(width, height) / 2 - (padding ?? CANVAS_PADDING);
  const out = [];
  for (let i = 0; i < n; i++) {
...
/** Grid layout: return array of [x, y] in pixel coords for rows×cols nodes (row-major: node i = r*cols+c). */
export function graphGridNodePositions(rows, cols, width, height, padding) {
  const pad = padding ?? CANVAS_PADDING;
  const w = width - 2 * pad;
  const h = height - 2 * pad;
  const cellW = cols > 1 ? w / (cols - 1) : w;
  const cellH = rows > 1 ? h / (rows - 1) : h;
  const out = [];
...
/** Max nodes for full grid rendering; larger grids show a placeholder instead. */
const GRID_DRAW_MAX_NODES = 400;

/**
 * Draw grid graph on canvas. edges = [u,v,w,...]; pathNodes = ordered path; source/target styled.
 * For 20x20, weight labels are omitted to avoid clutter. For grids > 400 nodes, draws a placeholder only.
 */
export function drawGridGraphOnCanvas(ctx, width, height, rows, cols, edges, pathNodes, distances, source, target) {
  const n = rows * cols;
  if (n > GRID_DRAW_MAX_NODES) {
    ...
    if (pathNodes != null && pathNodes.length > 0) {
      ...
      const cellW = cols > 1 ? w / (cols - 1) : w;
      const cellH = rows > 1 ? h / (rows - 1) : h;
      const toX = (node) => pad + (node % cols) * cellW;
      const toY = (node) => pad + Math.floor(node / cols) * cellH;
      ctx.strokeStyle = theme.barPos;
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(toX(pathNodes[0]), toY(pathNodes[0]));
      for (let i = 1; i < pathNodes.length; i++) {
        ctx.lineTo(toX(pathNodes[i]), toY(pathNodes[i]));
      }
      ctx.stroke();
      ctx.fillStyle = "#198754";
      ctx.beginPath();
      ctx.arc(toX(pathNodes[0]), toY(pathNodes[0]), 6, 0, 2 * Math.PI);
      ctx.fill();
      ctx.fillStyle = "#dc3545";
      ctx.beginPath();
      ctx.arc(toX(pathNodes[pathNodes.length - 1]), toY(pathNodes[pathNodes.length - 1]), 6, 0, 2 * Math.PI);
      ctx.fill();
    } else {
      ...
      ctx.fillText("Grid too large to draw (" + n + " nodes). See output below.", width / 2, height / 2);
    }
    return;
  }
  const positions = graphGridNodePositions(rows, cols, width, height, CANVAS_PADDING);
  const pathSet = new Set();
  for (let k = 0; k < pathNodes.length - 1; k++) {
    pathSet.add(pathNodes[k] + "," + pathNodes[k + 1]);
    pathSet.add(pathNodes[k + 1] + "," + pathNodes[k]);
  }
  ...
  const hasWeightRange = Number.isFinite(minW) && Number.isFinite(maxW) && maxW > minW;
  const weightRange = hasWeightRange ? maxW - minW : 1;

  function weightToColor(w) {
    if (!hasWeightRange) return "#94a3b8";
    const t = Number.isFinite(w) ? (w - minW) / weightRange : 0;
    const t1 = Math.max(0, Math.min(1, t));
    const r = Math.round(34 + (220 - 34) * t1);
    const g = Math.round(197 + (38 - 197) * t1);
    const b = Math.round(94 + (38 - 94) * t1);
    return "rgb(" + r + "," + g + "," + b + ")";
  }

  function formatWeight(w) {
    if (w >= 1000 && w % 1 === 0) return w >= 1e6 ? "1e" + Math.round(Math.log10(w)) : String(Math.round(w));
    if (Number(w) === w && w % 1 === 0) return String(w);
    return w.toFixed(1);
  }
  ...
  const pathColor = "#2563eb";
  const nodeStroke = theme.stroke;
  const showWeights = rows * cols <= 100;
  const drawn = new Set();
  ...
  for (let i = 0; i < edges.length; i += 3) {
    ...
    drawn.add(key);
    const onPath = pathSet.has(u + "," + v) || pathSet.has(v + "," + u);
    const [x1, y1] = positions[u];
    const [x2, y2] = positions[v];
    ctx.strokeStyle = onPath ? pathColor : weightToColor(w);
    ...
    if (showWeights) {
      ...
      ctx.fillStyle = w >= 500 ? "#1e293b" : "#475569";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      const mx = (x1 + x2) / 2;
      const my = (y1 + y2) / 2;
      ctx.fillText(formatWeight(w), mx, my);
    }
  }

  if (hasWeightRange && edges.length > 0 && n <= 100) {
    ctx.font = "11px \"DM Sans\", system-ui, sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "top";
    const legY = height - 20;
    ctx.fillStyle = theme.stroke;
    ctx.fillText("Weight: low ", 8, legY);
    const lowX = ctx.measureText("Weight: low ").width + 8;
    ctx.strokeStyle = weightToColor(minW);
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(lowX, legY + 4);
    ctx.lineTo(lowX + 24, legY + 4);
    ctx.stroke();
    ctx.fillStyle = theme.stroke;
    ctx.fillText(" → high ", lowX + 28, legY);
    const highX = lowX + 28 + ctx.measureText(" → high ").width;
    ctx.strokeStyle = weightToColor(maxW);
    ctx.beginPath();
    ctx.moveTo(highX, legY + 4);
    ctx.lineTo(highX + 24, legY + 4);
    ctx.stroke();
    ctx.fillText(" (" + formatWeight(minW) + "–" + formatWeight(maxW) + ")", highX + 28, legY);
  }

  const cellW = cols > 1 ? (width - 2 * CANVAS_PADDING) / (cols - 1) : 20;
  const cellH = rows > 1 ? (height - 2 * CANVAS_PADDING) / (rows - 1) : 20;
  const nodeRadius = Math.min(cellW, cellH) * 0.25;
  const showDistOnCells = rows * cols <= 25;

  for (let i = 0; i < n; i++) {
    const [x, y] = positions[i];
    let fill = theme.bg;
    if (i === source) fill = "#198754";
    else if (i === target) fill = "#dc3545";
    ctx.fillStyle = fill;
    ctx.strokeStyle = nodeStroke;
    ctx.lineWidth = i === source || i === target ? 2.5 : 1;
    ctx.beginPath();
    ctx.arc(x, y, Math.max(3, nodeRadius), 0, 2 * Math.PI);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = i === source || i === target ? "#fff" : nodeStroke;
    ctx.font = (rows * cols <= 25 ? "11px" : "9px") + " \"DM Sans\", system-ui, sans-serif";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ...
    if (i === source) {
      ...
      ctx.fillText("T", x, y);
    } else if (showDistOnCells && dist != null && dist !== Infinity && Number.isFinite(dist)) {
      ctx.fillText(dist % 1 === 0 ? String(dist) : dist.toFixed(0), x, y);
    }
  }
}
 ...
 * Mirrors mathlib colormap::height_to_rgb. h is clamped to [0, 1].
 * @param {number} h - value in [0, 1]
...
export function heightToRgb(h) {
  ...
  if (t < 0.5) {
    r = 0;
    g = Math.round(255 * 2 * t);
    b = 255;
  } else {
    r = Math.round(255 * 2 * (t - 0.5));
    g = 255;
    b = Math.round(255 * 2 * (1 - t));
  }
  return [r, g, b];
}
...
export function drawGraphOnCanvas(ctx, width, height, n, edges, pathNodes, distances, source) {
  const positions = graphNodePositions(n, width, height, CANVAS_PADDING);
  const pathSet = new Set();
  for (let k = 0; k < pathNodes.length - 1; k++)
    pathSet.add(pathNodes[k] + "," + pathNodes[k + 1]);
  ...
  const pathColor = theme.barPos;
  const nodeFill = theme.bg;
  const nodeStroke = theme.stroke;
  const nodeRadius = 14;
  const pathRadius = 16;
...
export function drawStorageGrid(ctx, width, height, rows, cols, storage, values) {
  ...
  const cellW = Math.min(60, (width - 2 * CANVAS_PADDING) / cols);
  const cellH = Math.min(50, (height - 2 * CANVAS_PADDING) / rows);
...
export function drawSparseCSRDiagram(canvasId, width, height, matrixRows, matrixCols, triplets) {
  ...
  const cellW = Math.min(36, (width - 2 * CANVAS_PADDING) / matrixCols);
  const cellH = Math.min(28, (height - 2 * CANVAS_PADDING - 60) / matrixRows);
...
export function drawVectorBarsGeneric(canvasId, c) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const val = c[i];
    const bh = val * scale;
    ctx.fillStyle = val >= 0 ? theme.barPos : theme.barNeg;
