/**
 * Neural WASM — Embedding space demo: text embeddings in 3D (PCA) and similar-doc list.
 * Loads mathlib from /mathlib/ for PCA; data from mathlib recommendation dataset.
 */
const needBuild = "Load /mathlib/ first. From repo root: just website-build then just website-serve.";
...
function scaleToCanvas(points, width, height, padding = 24) {
  ...
  return points.map((p) => [
    ...
    height - padding - ((p[1] - minY) / rangeY) * h,
  ]);
}
...
function getTopK(embeddings, queryIndex, k = 6) {
...
try {
  const lib = await import("/mathlib/pkg/mathlib.js");
  await lib.default();
  const { WasmMatrix, WasmPca } = lib;

  const resp = await fetch("/mathlib/recommendation/data.json");
  if (!resp.ok) throw new Error("Failed to load data.json from mathlib. Run: just website-build");
  ...
  let selectedIndex = 0;
  const canvas = byId("canvas-embed");
  const ctx = canvas.getContext("2d");
  const queryInput = byId("query-input");
  const listEl = byId("embed-list");
